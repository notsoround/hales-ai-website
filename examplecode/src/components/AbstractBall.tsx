...
const AbstractBall: React.FC<AbstractBallProps> = ({
  ...
  useEffect(() => {
    ...
    renderer.domElement.style.pointerEvents = 'auto'; // Changed from 'none' to enable interaction
    ...
    const handleMouseDown = () => {
      ...
      // Change colors
      gsap.to(uniformsRef.current.RGBr, { duration: 0.3, value: Math.random() * 10 });
      gsap.to(uniformsRef.current.RGBg, { duration: 0.3, value: Math.random() * 10 });
      gsap.to(uniformsRef.current.RGBb, { duration: 0.3, value: Math.random() * 10 });
    };
