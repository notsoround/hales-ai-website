...
import {acquireMicrophone} from '../src/components/cupcake/voiceInput.ts';
...
async function scenario({failStorage=false,auto=true,sample=128}={}){
 ...
 const scope={AudioContext,AbortController,acquireMicrophone,captureAcquisition:{current:null},captureGeneration:{current:0},setCaptureStarting:()=>{},setNotice:()=>{},meterFrame:{current:0},requestAnimationFrame:f=>{frames.push(f);return frames.length;},recording:false,voice:'idle',busy:false,micTesting:false,inputDevice:'default',microphoneConstraints:()=>({audio:true}),autoAnalyze:auto,title:'Synthetic',finishing:{current:false},mounted:{current:true},recorder:{current:null},streams:{current:[]},started:{current:0},context:{current:null},window:{MediaRecorder:Recorder},MediaRecorder:Recorder,navigator:{mediaDevices:{getUserMedia:async()=>({getTracks:()=>[],getAudioTracks:()=>[{label:"Synthetic microphone"}]})}},crypto:{randomUUID:()=> 'test-id'},setPreview:()=>{},setHeardAudio:v=>events.push(["audio",v]),setLevel:()=>{},setInputName:()=>{},setBusy:()=>{},setError:e=>events.push(['error',e]),setRecording:()=>{},setSeconds:()=>{},cleanupTracks:()=>{},putDraft:async d=>events.push(['draft',d.complete]),putChunk:async()=>{await new Promise(r=>setTimeout(r,5));if(failStorage)throw Error('quota');events.push(['chunk']);},refreshDrafts:async()=>events.push(['refresh']),upload:async d=>events.push(['upload',d.complete]),Blob,Date};
